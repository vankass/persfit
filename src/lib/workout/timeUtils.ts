...
export function getSessionBudgetSeconds(intensity: WorkoutIntensity, focus: WorkoutFocus, level: ProfileLevel): number {
  ...
  return Math.min(BUDGET_MAX_SECONDS, Math.max(BUDGET_MIN_SECONDS, minutes * 60));
}

export function estimateExerciseSeconds(prescription: WorkoutSetPrescription, category: string, intensity: WorkoutIntensity): number {
  ...
  return (sets * workPerSet) + (Math.max(0, sets - 1) * restSeconds) + EXERCISE_TRANSITION_SECONDS;
}

export function computeWorkoutDurationMinutes(exercises: PlannedExercise[], intensity: WorkoutIntensity): number {
  if (exercises.length === 0) return 0;

  let totalSeconds = 0;
  exercises.forEach((item, index) => {
    totalSeconds += estimateExerciseSeconds(item.prescription, item.exercise.category, intensity);
